...
@section('content')
    <div class="p-5">
        <h3>Starter Total Pompa ABT Dimanfaatkan Kabupaten</h3>
        <div class="d-flex align-items-center justify-content-between">
            <form action="{{ route('admin.starter.kabupaten.abt_dimanfaatkan') }}" method="GET" class="w-75 d-flex align-items-center gap-2 mb-3">
            ...
            <tbody>
                @forelse ($abt_dimanfaatkan as $ad)
                    <tr>
                        ...
                        <td>{{ $ad->total_unit }}</td>
                        <td>
                            <button class="btn btn-sm btn-warning" onclick="handleEdit('{{ route('admin.starter.kabupaten.abt_dimanfaatkan.update', Crypt::encryptString($ad->id)) }}', '{{ $ad->kabupaten->nama }}', {{ $ad->total_unit }})" data-bs-toggle="modal" data-bs-target="#exampleModal">Edit</button>
                        </td>
                    </tr>
                @empty
                    <tr>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item {{ $abt_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_dimanfaatkan', [...request()->query(), 'page' => $abt_dimanfaatkan->currentPage()-1]) }}" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li class="page-item {{ $abt_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_dimanfaatkan', [...request()->query(), 'page' => 1]) }}" aria-label="Previous">
                        <span aria-hidden="true">First</span>
                        </a>
                    </li>
                    @for ($i = 1; $i <= $abt_dimanfaatkan->lastPage(); $i++)
                        @if ($i>($abt_dimanfaatkan->currentPage()-5) && $i<($abt_dimanfaatkan->currentPage()+5))
                            <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$i?'active':'' }}"><a class="page-link" href="{{ route('admin.starter.kabupaten.abt_dimanfaatkan', [...request()->query(), 'page' => $i]) }}">{{ $i }}</a></li>
                        @endif
                    @endfor
                    <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$abt_dimanfaatkan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_dimanfaatkan', [...request()->query(), 'page' => $abt_dimanfaatkan->lastPage()]) }}" aria-label="Next">
                        <span aria-hidden="true">Last</span>
                        </a>
                    </li>
                    <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$abt_dimanfaatkan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_dimanfaatkan', [...request()->query(), 'page' => $abt_dimanfaatkan->currentPage()+1]) }}" aria-label="Next">
        ...
        <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="{{ route('admin.starter.kabupaten.abt_dimanfaatkan.store') }}" method="POST" id="edit-form" class="modal-content">
                    @csrf
                    <div class="modal-header">
