...
@section('content')
    <div class="p-5">
        <h3>Starter Total Pompa ABT Usulan Kabupaten</h3>
        <div class="d-flex align-items-center justify-content-between">
            <form action="{{ route('admin.starter.kabupaten.abt_usulan') }}" method="GET" class="w-75 d-flex align-items-center gap-2 mb-3">
            ...
            <div class="d-flex align-items center justify-content-end gap-2 w-50">
                ...
                <a href="{{ route('admin.starter.kabupaten.import.abt_usulan.view') }}" class="btn btn-primary">
            ...
            <tbody>
                @forelse ($abt_usulan as $au)
                    <tr>
                        ...
                        <td>{{ $au->kabupaten->nama }}</td>
                        <td>{{ $au->total_unit }}</td>
                        <td>
                            <button class="btn btn-sm btn-warning" onclick="handleEdit('{{ route('admin.starter.kabupaten.abt_usulan.update', Crypt::encryptString($au->id)) }}', '{{ $au->kabupaten->nama }}', {{ $au->total_unit }})" data-bs-toggle="modal" data-bs-target="#exampleModal">Edit</button>
                        </td>
                    </tr>
                @empty
                    <tr>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item {{ $abt_usulan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_usulan', [...request()->query(), 'page' => $abt_usulan->currentPage()-1]) }}" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li class="page-item {{ $abt_usulan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_usulan', [...request()->query(), 'page' => 1]) }}" aria-label="Previous">
                        <span aria-hidden="true">First</span>
                        </a>
                    </li>
                    @for ($i = 1; $i <= $abt_usulan->lastPage(); $i++)
                        @if ($i>($abt_usulan->currentPage()-5) && $i<($abt_usulan->currentPage()+5))
                            <li class="page-item {{ $abt_usulan->currentPage()==$i?'active':'' }}"><a class="page-link" href="{{ route('admin.starter.kabupaten.abt_usulan', [...request()->query(), 'page' => $i]) }}">{{ $i }}</a></li>
                        @endif
                    @endfor
                    <li class="page-item {{ $abt_usulan->currentPage()==$abt_usulan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_usulan', [...request()->query(), 'page' => $abt_usulan->lastPage()]) }}" aria-label="Next">
                        <span aria-hidden="true">Last</span>
                        </a>
                    </li>
                    <li class="page-item {{ $abt_usulan->currentPage()==$abt_usulan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('admin.starter.kabupaten.abt_usulan', [...request()->query(), 'page' => $abt_usulan->currentPage()+1]) }}" aria-label="Next">
        ...
        <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="{{ route('admin.starter.kabupaten.abt_usulan.store') }}" method="POST" id="edit-form" class="modal-content">
                    @csrf
                    <div class="modal-header">
