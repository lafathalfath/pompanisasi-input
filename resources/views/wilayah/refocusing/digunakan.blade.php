...
<div class="d-flex flex-col justify-content-center">
    <div>
        <br>
        <form action="{{ route('wilayah.pompa.ref.digunakan') }}" method="GET" id="form-filter" class="mb-3" style="display: flex; justify-content: space-between; gap: 10px; align-items: center;" >
            ...
            <a href="{{ route('wilayah.pompa.ref.digunakan') }}" class="btn btn-secondary">Reset</a>
        </form>
        <table class="w-100 table table-bordered">
            <thead>
                <tr>
                    ...
                    <th colspan="3" class="text-center">Pompa Refocusing <br>Dimanfaatkan</th>
                    <th rowspan="2">Total <br>dimanfaatkan <br>(unit)</th>
            ...
            <tbody>
                @forelse ($ref_dimanfaatkan as $rd)
                    <tr>
                        ...
                        <td><a href="{{ route('wilayah.pompa.ref.digunakan.detail', Crypt::encryptString($rd->id)) }}" class="btn btn-sm btn-info">Detail</a></td>
                    </tr>
                @empty
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item {{ $ref_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('wilayah.pompa.ref.digunakan', [...request()->query(), 'page' => $ref_dimanfaatkan->currentPage()-1]) }}" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li class="page-item {{ $ref_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('wilayah.pompa.ref.digunakan', [...request()->query(), 'page' => 1]) }}" aria-label="Previous">
                        <span aria-hidden="true">First</span>
                        </a>
                    </li>
                    @for ($i = 1; $i <= $ref_dimanfaatkan->lastPage(); $i++)
                        @if ($i>($ref_dimanfaatkan->currentPage()-5) && $i<($ref_dimanfaatkan->currentPage()+5))
                            <li class="page-item {{ $ref_dimanfaatkan->currentPage()==$i?'active':'' }}"><a class="page-link" href="{{ route('wilayah.pompa.ref.digunakan', [...request()->query(), 'page' => $i]) }}">{{ $i }}</a></li>
                        @endif
                    @endfor
                    <li class="page-item {{ $ref_dimanfaatkan->currentPage()==$ref_dimanfaatkan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('wilayah.pompa.ref.digunakan', [...request()->query(), 'page' => $ref_dimanfaatkan->lastPage()]) }}" aria-label="Next">
                        <span aria-hidden="true">Last</span>
                        </a>
                    </li>
                    <li class="page-item {{ $ref_dimanfaatkan->currentPage()==$ref_dimanfaatkan->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('wilayah.pompa.ref.digunakan', [...request()->query(), 'page' => $ref_dimanfaatkan->currentPage()+1]) }}" aria-label="Next">
