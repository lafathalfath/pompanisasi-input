          ...
          @forelse ($abt_usulan as $au)
            <tr>
                ...
                <td class="border-0 d-flex align-items-center justify-content-center gap-2">
                  @if ($au->verified_at)
                    <button title="batalkan verifikasi" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#unverifModal" onclick="handleUnverif('{{ route('kabupaten.verif.abt.usulan.unverif', Crypt::encryptString($au->id)) }}')"><span>&#10005;</span></button>
                  @elseif (!$au->verified_at)
