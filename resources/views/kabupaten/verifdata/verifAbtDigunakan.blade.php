...
@section('content')
  ...
  <div class="container mt-4">
    <h2>Verifikasi Data Pompa ABT Digunakan</h2>
    <table class="table table-bordered">
      <thead>
        <tr>
          ...
          <th colspan="3">Pompa ABT Digunakan</th>
          ...
          @forelse ($abt_dimanfaatkan as $ad)
            <tr>
                ...
                <td class="border-0 d-flex align-items-center justify-content-center gap-2">
                  @if ($ad->verified_at)
                    <button title="batalkan verifikasi" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#unverifModal" onclick="handleUnverif('{{ route('kabupaten.verif.abt.digunakan.unverif', Crypt::encryptString($ad->id)) }}')"><span>&#10005;</span></button>
                  @elseif (!$ad->verified_at)
                    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#verifModal" onclick="handleClick('{{ route('kabupaten.verif.abt.digunakan.verif', Crypt::encryptString($ad->id)) }}')"><span>&#10003;</span></button>
                  @endif
                </td>
            </tr>
          @empty
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
            <li class="page-item {{ $abt_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                <a class="page-link" href="{{ route('kabupaten.verif.abt.digunakan.view', ['kecamatan' => request()->query('kecamatan'), 'page' => $abt_dimanfaatkan->currentPage()-1]) }}" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <li class="page-item {{ $abt_dimanfaatkan->currentPage()==1?'disabled':'' }}">
                <a class="page-link" href="{{ route('kabupaten.verif.abt.digunakan.view', ['kecamatan' => request()->query('kecamatan'), 'page' => 1]) }}" aria-label="Previous">
                <span aria-hidden="true">First</span>
                </a>
            </li>
            @for ($i = 1; $i <= $abt_dimanfaatkan->lastPage(); $i++)
                @if ($i>($abt_dimanfaatkan->currentPage()-5) && $i<($abt_dimanfaatkan->currentPage()+5))
                    <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$i?'active':'' }}"><a class="page-link" href="{{ route('kabupaten.verif.abt.digunakan.view', ['kecamatan' => request()->query('kecamatan'), 'page' => $i]) }}">{{ $i }}</a></li>
                @endif
            @endfor
            <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$abt_dimanfaatkan->lastPage()?'disabled':'' }}">
                <a class="page-link" href="{{ route('kabupaten.verif.abt.digunakan.view', ['kecamatan' => request()->query('kecamatan'), 'page' => $abt_dimanfaatkan->lastPage()]) }}" aria-label="Next">
                <span aria-hidden="true">Last</span>
                </a>
            </li>
            <li class="page-item {{ $abt_dimanfaatkan->currentPage()==$abt_dimanfaatkan->lastPage()?'disabled':'' }}">
                <a class="page-link" href="{{ route('kabupaten.verif.abt.digunakan.view', ['kecamatan' => request()->query('kecamatan'), 'page' => $abt_dimanfaatkan->currentPage()+1]) }}" aria-label="Next">
