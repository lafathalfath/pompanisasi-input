...
<div class="mx-5 d-flex flex-col justify-content-center">
    <div>
        <br>
        <form action="{{ route('nasional.pompa.ref.diterima') }}" method="GET" class="mb-3" id="form-filter" style="display: flex; justify-content: space-between; gap: 10px; align-items: center;" >
            ...
            <a href="{{ route('nasional.pompa.ref.diterima') }}" class="btn btn-secondary">Reset</a>
        </form>
            ...
            <tbody>
                @forelse ($ref_diterima as $rd)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('nasional.pompa.ref.diterima.detail', Crypt::encryptString($rd->id)) }}" class="btn btn-sm btn-info">Detail</a>
                        </td>
                    </tr>
                @empty
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item {{ $ref_diterima->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('nasional.pompa.ref.diterima', [...request()->query(), 'page' => $ref_diterima->currentPage()-1]) }}" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li class="page-item {{ $ref_diterima->currentPage()==1?'disabled':'' }}">
                        <a class="page-link" href="{{ route('nasional.pompa.ref.diterima', [...request()->query(), 'page' => 1]) }}" aria-label="Previous">
                        <span aria-hidden="true">First</span>
                        </a>
                    </li>
                    @for ($i = 1; $i <= $ref_diterima->lastPage(); $i++)
                        @if ($i>($ref_diterima->currentPage()-5) && $i<($ref_diterima->currentPage()+5))
                            <li class="page-item {{ $ref_diterima->currentPage()==$i?'active':'' }}"><a class="page-link" href="{{ route('nasional.pompa.ref.diterima', [...request()->query(), 'page' => $i]) }}">{{ $i }}</a></li>
                        @endif
                    @endfor
                    <li class="page-item {{ $ref_diterima->currentPage()==$ref_diterima->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('nasional.pompa.ref.diterima', [...request()->query(), 'page' => $ref_diterima->lastPage()]) }}" aria-label="Next">
                        <span aria-hidden="true">Last</span>
                        </a>
                    </li>
                    <li class="page-item {{ $ref_diterima->currentPage()==$ref_diterima->lastPage()?'disabled':'' }}">
                        <a class="page-link" href="{{ route('nasional.pompa.ref.diterima', [...request()->query(), 'page' => $ref_diterima->currentPage()+1]) }}" aria-label="Next">
